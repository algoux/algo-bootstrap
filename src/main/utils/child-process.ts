...
export function spawn(
  ...
  return _spawn(cmd, args, {
    ...
    .catch((e) => {
      logProcess.error(
        type,
        `[spawn.error ${Date.now() - __start + 'ms'}]`,
        cmd,
        args.join(' '),
        '\nerror:',
        e,
...
export function execFile(
  ...
  return _execFile(file, args, {
    ...
    .then((r) => {
      logProcess.info(
        type,
        `[execFile.done ${Date.now() - __start + 'ms'}]`,
        file,
        args.join(' '),
        '\nstdout:',
        r.stdout,
    ...
    .catch((e) => {
      logProcess.error(
        type,
        `[execFile.error ${Date.now() - __start + 'ms'}]`,
        file,
        args.join(' '),
        '\nerror:',
        e,
    ...
    sudo.exec(
      ...
      (e, stdout, stderr) => {
        if (e) {
          logProcess.error(
            type,
            `[sudoExec.error ${Date.now() - __start + 'ms'}]`,
            cmd,
            '\nerror:',
            e,
          );
          ...
          logProcess.info(
            type,
            `[sudoExec.done ${Date.now() - __start + 'ms'}]`,
            cmd,
            '\nstdout:',
            stdout,
            '\nstderr:',
